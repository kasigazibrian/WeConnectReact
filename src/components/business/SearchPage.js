...
export default class SearchPage extends React.Component {
  ...
  handleSearchInputChange = event => {
	if (event.target.value === "Location" || event.target.value === "Category") {
		  ...
		  <Container>
			...
			<form onSubmit={this.handleSubmit}>
			  <Navbar color="dark" light expand="md">
				<NavbarBrand href="/search" className="text-light"><img src={logo} alt="We Connect!" height="45px"
																		width="42px"/>Search for
				  Businesses</NavbarBrand>
				<NavbarToggler onClick={this.toggle}/>
				<Collapse isOpen={this.state.isOpen} navbar>
				  <Nav className="ml-auto" navbar>
					<NavItem className='nav-item-name-properties'>
					  <input onChange={this.handleSearchInputChange} name="businessName"
							 placeholder="Enter Business Name" className="my-input" type="text" id="components"/>
					</NavItem>
					<NavItem className='nav-item-name-properties'>
					  <label className='my-labels'>Filter By:</label>
					</NavItem>
					<NavItem className='nav-item-name-properties'>
					  <Input style={{marginTop: "12px"}} onChange={this.handleSearchInputChange} type="select"
							 name="category" id="exampleSelect">
					...
					<NavItem className='nav-item-name-properties'>
					  <Input style={{marginTop: "12px"}} onChange={this.handleSearchInputChange} type="select"
							 name="location" id="exampleSelect">
