...
class BusinessProfile extends React.Component {
  ...
  render() {
	...
	let review_list = reviews.map((review, index) => (<ul className="review" key={index}>
	  <li dangerouslySetInnerHTML={this.createMarkup(review)}>
	  </li>
	  <li className="profile-labels">Added on: {review.date_created}</li>
	  <br/>
	</ul>));
				...
				<div className="content-left">
				  <FormGroup>
					...
					<div className={createEditPermission}>
					  ...
					  <Button style={{marginLeft: "5px"}} onClick={this.toggle} className="btn-danger">
						<Trash/></Button>
					</div>
				  </FormGroup>
				  <FormGroup>
				  ...
				  <div className="content-right">
					<form id="add-review" onSubmit={this.handleSubmit}>
					  <div className={addReviewPermission}>
						...
						<FormGroup>
						  <Button style={{borderRadius: "20px"}} className={"btn btn-lg btn-info btn-block"}>Add
							Review </Button>
						</FormGroup>
					  </div>
					  <FormGroup>
